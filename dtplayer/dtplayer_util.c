...
int player_host_init(dtplayer_context_t * dtp_ctx)
{
	...
	if (pctrl->has_audio) {
		...
		dt_info(TAG,"[%s:%d] audio extra_size:%d time_base.num:%d den:%d\n",
		       __FUNCTION__, __LINE__, host_para->audio_extra_size,
		       host_para->audio_num, host_para->audio_den);
	...
	if (pctrl->has_video) {
		...
		host_para->video_dest_width = vstream->width;
		host_para->video_dest_height = vstream->height;
		...
		dt_info(TAG,"[%s:%d]format:%d width:%d height:%d fmt:%d rate:%d ratio:%d fps:%g \n",
		     __FUNCTION__, __LINE__, host_para->video_format,
		     host_para->video_src_width, host_para->video_src_height,
		     host_para->video_src_pixfmt, host_para->video_rate,
		     host_para->video_ratio, host_para->video_fps);
		if (host_para->video_extra_size > 0) {
			memset(host_para->video_extra_data, 0,
