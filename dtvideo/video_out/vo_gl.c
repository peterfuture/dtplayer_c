...
static void init_texture(void)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, dw, dh,
...
static int vo_gl_init(void)
{
	 ...
	 *-----------------------------------------------------------------------------*/
	{
		...
		gl_vinfo =
		    glXChooseVisual(XDisplay, DefaultScreen(XDisplay),
				    attributeList);
		...
		cmap =
		    XCreateColormap(XDisplay,
				    RootWindow(XDisplay, gl_vinfo->screen),
		...
		win =
		    XCreateWindow(XDisplay,
				  RootWindow(XDisplay, gl_vinfo->screen), 0, 0,
				  dw, dh, 0, gl_vinfo->depth, InputOutput,
				  gl_vinfo->visual,
				  CWBorderPixel | CWColormap | CWEventMask,
				  &swa);
#define WIN_TITLE "Dawn Light Player"
		XSetStandardProperties(XDisplay, win, WIN_TITLE, WIN_TITLE,
				       None, NULL, 0, NULL);
...
static int vo_gl_vfmt2rgb(AVPicture * dst, AVPicture * src)
{
	...
	img_convert_ctx = sws_getCachedContext(img_convert_ctx,
					       dw, dh, src_pic_fmt,
					       dw, dh, my_pic_fmt, SWS_BICUBIC,
					       NULL, NULL, NULL);

	sws_scale(img_convert_ctx, src->data, src->linesize,
		  0, dh, dst->data, dst->linesize);
...
static void update_texture(void)
{
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, texName);
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, dw, dh,
