...
static int DxCreateWindow(void)
{
	...
	if (g_vidmode)
		g_hwnd =
		    CreateWindowEx(WS_EX_TOPMOST, WND_CLASSNAME, "", WS_POPUP,
				   CW_USEDEFAULT, CW_USEDEFAULT, 100, 100, NULL,
				   NULL, hInstance, NULL);
	else
		g_hwnd =
		    CreateWindowEx(0, WND_CLASSNAME, "",
	...
	g_fs_hwnd =
	    CreateWindow(FS_CLASSNAME, "", WS_POPUP, 0, 0, g_screen_width,
...
static int DxInitDirectDraw(void)
{
	...
	if (hddraw_dll == NULL) {
		av_log(NULL, AV_LOG_ERROR,
		       "vo_directx: failed loading ddraw.dll\n");
		return FALSE;
	}

	OurDirectDrawCreateEx =
	    (void *)GetProcAddress(hddraw_dll, "DirectDrawCreateEx");
	...
	if (OurDirectDrawCreateEx
	    (NULL, (VOID **) & g_lpdd, &IID_IDirectDraw7, NULL) != DD_OK) {
	...
	if (g_vidmode) {
		if (IDirectDraw_SetCooperativeLevel
			...
			av_log(NULL, AV_LOG_ERROR,
			       "vo_dx: can't set cooperative\n");
			return FALSE;
		}
	} else {
		if (IDirectDraw_SetCooperativeLevel
		    (g_lpdd, g_hwnd, DDSCL_NORMAL) != DD_OK) {
			av_log(NULL, AV_LOG_ERROR,
			       "vo_dx: can't set cooperative\n");
			return FALSE;
		}
...
static int DxCreatePrimarySurface(void)
{
	...
	if (IDirectDraw_CreateSurface(g_lpdd, &ddsd, &g_lpddsPrimary, NULL) ==
	    DD_OK)
		return TRUE;
	return FALSE;
}
...
static int DxCreateOverlay(void)
{
	...
	for (i = 0; i < NUM_FORMATS; i++) {
		...
		if (IDirectDraw_CreateSurface
		    (g_lpdd, &ddsdOverlay, &g_lpddsOverlay, NULL) == DD_OK) {
			g_image_format = g_ddpf[i].img_format;
			av_log(NULL, AV_LOG_INFO,
			       "vo_dx: overlay with format %s created\n",
			       g_ddpf[i].img_format_name);

			/* get the surface directly attached to the primary (the back buffer) */
			ddsdOverlay.ddsCaps.dwCaps = DDSCAPS_BACKBUFFER;
			if (IDirectDrawSurface_GetAttachedSurface
			    (g_lpddsOverlay, &ddsdOverlay.ddsCaps,
			     &g_lpddsBack) != DD_OK) {
				av_log(NULL, AV_LOG_ERROR,
				       "vo_dx: can't get back surface from overlay\n");
				return FALSE;
			}
			return TRUE;
		}
	...
	for (i = 0; i < NUM_FORMATS; i++) {
		ddsdOverlay.ddpfPixelFormat = g_ddpf[i].g_ddpfOverlay;
		if (DD_OK ==
		    g_lpdd->lpVtbl->CreateSurface(g_lpdd, &ddsdOverlay,
						  &g_lpddsOverlay, NULL)) {
			g_image_format = g_ddpf[i].img_format;
			av_log(NULL, AV_LOG_INFO,
			       "vo_dx: overlay with format %s created\n",
...
static int DxGetCaps(void)
{
	...
	if (capsDrv.dwCaps & DDCAPS_OVERLAYSTRETCH)
		g_StretchFactor1000 =
		    capsDrv.dwMinOverlayStretch >
		    1000 ? capsDrv.dwMinOverlayStretch : 1000;
	else
	...
	g_align_dst =
	    (capsDrv.dwCaps & DDCAPS_ALIGNSIZEDEST) ? capsDrv.
	    dwAlignSizeDest : 0;
	g_boundary_src =
	    (capsDrv.dwCaps & DDCAPS_ALIGNBOUNDARYSRC) ? capsDrv.
	    dwAlignBoundarySrc : 0;
	g_boundary_dst =
	    (capsDrv.dwCaps & DDCAPS_ALIGNBOUNDARYDEST) ? capsDrv.
...
static int DxManageOverlay(void)
{
	...
	if (g_align_dst) {
		rd.right =
		    ((rd.right + g_align_dst - 1) / g_align_dst) * g_align_dst;
	}
	...
	ddrval =
	    IDirectDrawSurface_UpdateOverlay(g_lpddsOverlay, &rs,
					     g_lpddsPrimary, &rd, dwUpdateFlags,
					     &ovfx);
	/*ok we can't do anything about it -> hide overlay */
	if (ddrval != DD_OK) {
		av_log(NULL, AV_LOG_ERROR,
		       "update overlay faild , hide overlay\n");
		IDirectDrawSurface_UpdateOverlay(g_lpddsOverlay, NULL,
						 g_lpddsPrimary, NULL,
						 DDOVER_HIDE, NULL);
		return FALSE;
	}
...
static void flip_page(void)
{
	...
	if (g_doublebuffer) {
		dxresult =
		    IDirectDrawSurface_Flip(g_lpddsOverlay, NULL, DDFLIP_WAIT);
		if (dxresult == DDERR_SURFACELOST) {
			av_log(NULL, AV_LOG_INFO,
			       "vo_directx: Lost, Restoring Surface\n");
			IDirectDrawSurface_Restore(g_lpddsBack);
			IDirectDrawSurface_Restore(g_lpddsOverlay);
			IDirectDrawSurface_Restore(g_lpddsPrimary);
			dxresult =
			    IDirectDrawSurface_Flip(g_lpddsOverlay, NULL,
						    DDFLIP_WAIT);
		}
		if (dxresult != DD_OK)
			av_log(NULL, AV_LOG_ERROR,
			       "vo_directx: can not flip page\n");
	} else {
		DDBLTFX ddbltfx;
		memset(&ddbltfx, 0, sizeof(DDBLTFX));
		ddbltfx.dwSize = sizeof(DDBLTFX);
		ddbltfx.dwDDFX = DDBLTFX_NOTEARING;
		IDirectDrawSurface_Blt(g_lpddsPrimary, &rd, g_lpddsBack, NULL,
				       DDBLT_WAIT, &ddbltfx);
	}
	...
	if (IDirectDrawSurface_Lock
	    (g_lpddsBack, NULL, &ddsd, DDLOCK_NOSYSLOCK | DDLOCK_WAIT,
	     NULL) == DD_OK) {
		if (tmp_image) {
			free(tmp_image);
			tmp_image = NULL;
		...
		if (!tmp_image) {
			...
			tmp_image = g_image =
			    calloc(1, g_image_height * g_dstride * 2);
		}
	}
}
...
static int vo_dx_init(void)
{
	...
	if (-1 ==
	    avpicture_alloc(my_pic, g_image_format, g_image_width,
			    g_image_height)) {
...
static int vo_dx_vfmt2rgb(AVPicture * dst, AVPicture * src)
{
	...
	img_convert_ctx = sws_getCachedContext(img_convert_ctx,
					       dlpctxp->pwidth,
					       dlpctxp->pheight,
					       dlpctxp->pixfmt, dlpctxp->pwidth,
					       dlpctxp->pheight, g_image_format,
					       0, NULL, NULL, NULL);

	sws_scale(img_convert_ctx, src->data, src->linesize,
		  0, dlpctxp->pheight, dst->data, dst->linesize);
...
static void vo_dx_display(AVPicture * pic)
{
	...
	memcpy(g_image, my_pic->data[0],
	       my_pic->linesize[0] * dlpctxp->pheight);
	flip_page();
}
...
static int vo_dx_control(int cmd, void *arg)
{
	...
	default:
		av_log(NULL, AV_LOG_ERROR,
		       "vo directx: cmd not support now!\n");
		break;
	}
	return 0;
}
