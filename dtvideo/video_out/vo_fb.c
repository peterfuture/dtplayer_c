...
static int vo_fb_init(void)
{
	...
	if (fbctxp->fb < 0) {
		av_log(NULL, AV_LOG_ERROR,
		       "Error opening %s: %m. Check kernel config\n",
		       fbctxp->dev);
		return -1;
	}
	...
	fbctxp->mem =
	    fbctxp->orig_mem + (fbctxp->fixinfo.line_length * fbctxp->dy)
	...
	if (-1 ==
	    avpicture_alloc(my_pic, fbctxp->pixfmt, fbctxp->dw, fbctxp->dh)) {
...
static int vo_fb_vfmt2rgb(AVPicture * dst, AVPicture * src)
{
	...
	img_convert_ctx = sws_getCachedContext(img_convert_ctx,
					       dlpctxp->pwidth,
					       dlpctxp->pheight,
					       dlpctxp->pixfmt, fbctxp->dw,
					       fbctxp->dh, fbctxp->pixfmt,
					       SWS_BICUBIC, NULL, NULL, NULL);

	sws_scale(img_convert_ctx, src->data, src->linesize,
		  0, fbctxp->dh, dst->data, dst->linesize);
...
static void vo_fb_display(AVPicture * pic)
{
	...
	for (i = 0; i < fbctxp->dh; i++) {
		memcpy(dst, src,
		       fbctxp->dw * (fbctxp->varinfo.bits_per_pixel / 8));
...
static void toggle_full_screen(void)
{
	...
	if (-1 ==
	    avpicture_alloc(my_pic, fbctxp->pixfmt, fbctxp->dw, fbctxp->dh)) {
		av_log(NULL, AV_LOG_ERROR,
		       "toggle full: avpicture alloc error\n");
		dlp_exit(-1);
	}
