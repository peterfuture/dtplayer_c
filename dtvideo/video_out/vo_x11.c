...
static int vo_x11_init()
{
	...
	XSetStandardProperties(Xdisplay, Xvowin, Xtitle, Xtitle, None, NULL, 0,
			       &hint);
	...
	Ximg = XCreateImage(Xdisplay, xvinfo.visual,
			    Xdepth, ZPixmap, 0, NULL, dw, dh, 8, 0);

	{
		int i, fontcount;
		char **list;
		list =
		    XListFonts(Xdisplay, "-*-helvetica-*-*-*-*-0-0-*-*-*-0-*-*",
			       200, &fontcount);
		for (i = 0; i < fontcount; i++) {
			if (NULL == Xfont_120)
				Xfont_120 =
				    LoadQueryScalableFont(Xdisplay, Xscreen,
							  list[i], 120);
			if (NULL == Xfont_240)
				Xfont_240 =
				    LoadQueryScalableFont(Xdisplay, Xscreen,
